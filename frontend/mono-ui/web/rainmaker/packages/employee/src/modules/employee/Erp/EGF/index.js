...
class EGFFinance extends Component {
  ...
  componentDidUpdate(prevProps) {
    const { actionListArr, location } = this.props;
    const currentUrl = location.pathname;

    // 1. Sync Locale Cookie for the Iframe
    ...
    // 2. TRIGGER ON URL CHANGE 
    // (When user clicks a new sidebar menu item)
    if (currentUrl !== prevProps.location.pathname) {
      console.log("URL Changed, re-loading iframe...");
      this.loadFinanceIframe();
    }
    else if (
      actionListArr &&
      actionListArr.length > 0 &&
      (!prevProps.actionListArr || prevProps.actionListArr.length === 0)
    ) {
      console.log("Redux menu populated after refresh, triggering validation...");
      this.loadFinanceIframe();
    }
  }
  ...
  loadFinanceIframe() {
    const { actionListArr, location, history } = this.props;
    const currentPath = location.pathname;

    if (!actionListArr || actionListArr.length === 0) {
      console.log("Waiting for Redux menu to populate...");
      return;
    }
    const isAuthorized = actionListArr.some(action => {
      const cleanActionUrl = (action.url || "").replace(/^\/|\/$/g, '');
      const cleanNavUrl = (action.navigationURL || "").replace(/^\/|\/$/g, '');
      const cleanCurrentPath = currentPath.replace(/^\/|\/$/g, '');

      return (
        (cleanActionUrl !== "" && cleanCurrentPath.includes(cleanActionUrl)) ||
        (cleanNavUrl !== "" && cleanCurrentPath.includes(cleanNavUrl))
      );
    });

    if (!isAuthorized) {
      console.error(`Security Block: ${currentPath} is not authorized.`);
      alert("Access Denied: You do not have permission for this module.");
      history.push("/inbox");
      return;
    }
    const erp_url = this.getFinanceIframeURL(currentPath);

    this.setState({ isLoading: true, lastUrl: currentPath }, () => {
...
const mapStateToProps = (state) => {
  return {
    actionListArr: state.app.menu || [],
  };
};
export default connect(mapStateToProps, { setSessionTTL, logout })(EGFFinance);