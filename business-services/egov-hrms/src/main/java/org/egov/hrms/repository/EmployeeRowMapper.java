	...
	public void setAssignments(ResultSet rs, Employee currentEmployee) {
		try {
			...
			if(!StringUtils.isEmpty(rs.getString("assignment_uuid")) && !ids.contains(rs.getString("assignment_uuid"))) {
				...
				.designation(rs.getString("assignment_designation")).division(rs.getString("assignment_division")).fromDate(rs.getLong("assignment_fromdate")).toDate(null == rs.getObject("assignment_todate")? null : rs.getLong("assignment_todate"))
